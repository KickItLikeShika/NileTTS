...
class GPTTrainer(BaseTTS):
    ...
    def test_run(self, assets) -> Tuple[Dict, Dict]:  # pylint: disable=W0613
        ...
        if self.config.test_sentences:
            ...
            for idx, s_info in enumerate(self.config.test_sentences):
                ...
                ref_audios.append(s_info["speaker_wav"])
                ref_texts.append(s_info["text"])

            # delete inference layers
            del self.xtts.gpt.gpt_inference
            del self.xtts.gpt.gpt.wte
            
            if self.config.compute_eval_metrics and generated_wavs:
                metrics = self._compute_eval_metrics(generated_wavs, ref_texts, ref_audios)
        
        return {"audios": test_audios, "metrics": metrics}
    
    def _compute_eval_metrics(self, generated_wavs, ref_texts, ref_audios):
        metrics = {}
        wer_scores = []
        cer_scores = []
        similarity_scores = []
        
        print("computing WER and CER metrics")
        whisper_model = whisper.load_model(self.config.whisper_model_name)
        
        for idx, (wav, ref_text) in enumerate(zip(generated_wavs, ref_texts)):
            try:
                with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
                    wav_tensor = torch.tensor(wav).unsqueeze(0)
                    torchaudio.save(f.name, wav_tensor, self.args.output_sample_rate)
                    result = whisper_model.transcribe(f.name, language="ar")
                ...
                print(f"error computing WER or CER for sample {idx}: {e}")
        
        del whisper_model
        torch.cuda.empty_cache()
        
        if wer_scores:
            metrics["wer"] = float(np.mean(wer_scores))
            metrics["cer"] = float(np.mean(cer_scores))
        ...
        for idx, (wav, ref_audio) in enumerate(zip(generated_wavs, ref_audios)):
            try:
                with tempfile.NamedTemporaryFile(suffix=".wav", delete=False) as f:
                    wav_tensor = torch.tensor(wav).unsqueeze(0)
                    torchaudio.save(f.name, wav_tensor, self.args.output_sample_rate)
                    
                    emb_gen = speaker_encoder.encode_batch(speaker_encoder.load_audio(f.name)).squeeze().cpu().numpy()
                    emb_ref = speaker_encoder.encode_batch(speaker_encoder.load_audio(ref_audio)).squeeze().cpu().numpy()
                    similarity = float(np.dot(emb_gen, emb_ref) / (np.linalg.norm(emb_gen) * np.linalg.norm(emb_ref)))
                    similarity_scores.append(similarity)
                    os.unlink(f.name)
            except Exception as e:
                print(f" | > Error computing similarity for sample {idx}: {e}")
        
        del speaker_encoder
        torch.cuda.empty_cache()
        
        if similarity_scores:
            metrics["speaker_similarity"] = float(np.mean(similarity_scores))
    ...
    def test_log(
        ...
        if "metrics" in outputs and outputs["metrics"]:
            ...
            print(f"eval metrics at step {steps}:")
            ...
            wandb.log(eval_metrics, step=steps)
